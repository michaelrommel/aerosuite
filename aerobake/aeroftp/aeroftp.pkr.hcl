...
build {
  ...
  # Install the aeroslot OpenRC service
  provisioner "file" {
    source      = "./_etc_init.d_aeroslot"
    destination = "/tmp/_etc_init.d_aeroslot"
  }
  provisioner "file" {
    source      = "./_etc_conf.d_aeroslot"
    destination = "/tmp/_etc_conf.d_aeroslot"
  }

  provisioner "shell" {
    inline = [
      "sudo mv /tmp/_etc_conf.d_aeroslot /etc/conf.d/aeroslot",
      "sudo mv /tmp/_etc_init.d_aeroslot /etc/init.d/aeroslot",
      "sudo chmod +x /etc/init.d/aeroslot",
      "sudo rc-update add aeroslot default",
    ]
  }
  ...
  provisioner "file" {
    source      = "./_etc_conf.d_aeroftp-routing"
    destination = "/tmp/_etc_conf.d_aeroftp-routing"
  }

  provisioner "shell" {
    inline = [
      "sudo mv /tmp/_etc_conf.d_aeroftp-routing /etc/conf.d/aeroftp-routing",
