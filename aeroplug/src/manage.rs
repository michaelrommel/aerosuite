...
use aerocore::{aws_query, extract_balanced, extract_scalar, fetch_imds_credentials, fetch_imds_instance_id, AwsCredentials};
...
async fn cmd_attach(args: &Args, instance_id: &str, creds: &AwsCredentials) -> Result<()> {
    let (eni_id, _, private_ip) =
        find_slot_eni(&args.region, creds, args.slot, "available").await?;
    ...
    if let Some(ip) = private_ip {
        let ip_file = Path::new("/var/run/slotmanager/ip");
        if let Some(dir) = ip_file.parent() {
            fs::create_dir_all(dir)
                .with_context(|| format!("Failed to create directory {}", dir.display()))?;
        }
        fs::write(ip_file, &ip)
            .with_context(|| format!("Failed to write private IP to {}", ip_file.display()))?;
        println!("   Private IP written to {}", ip_file.display());
    }
...
async fn cmd_detach(args: &Args, _instance_id: &str, creds: &AwsCredentials) -> Result<()> {
    ...
    println!(
        "  Found attached ENI for slot {}: {eni_id}  (attachment: {attachment_id})",
        args.slot
    );
...
///
/// Returns `(eni_id, Option<attachment_id>, Option<private_ip>)`.
...
async fn find_slot_eni(
    ...
    let private_ip = extract_scalar(item_xml, "privateIpAddress").map(|s| {
        println!("   Private IP: {s}");
        s.to_string()
    });
    ...
    Ok((eni_id, attachment_id, private_ip))
}
