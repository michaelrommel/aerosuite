//! Shared AWS utilities: SigV4 signing and IMDSv2 credential fetching.
...
/// Obtain an IMDSv2 session token.  All subsequent metadata requests must
/// carry this token in the `X-aws-ec2-metadata-token` header.
...
///
/// The caller must:
/// - Put all API parameters in the POST body as `application/x-www-form-urlencoded`
/// - Add `x-amz-date` and `Authorization` headers from the returned struct
/// - Add `x-amz-security-token` separately when `session_token` is present
pub fn sigv4_sign(
    method: &str,
    host: &str,
    path: &str,
...
pub async fn aws_query(
    ...
    println!("[debug] POST {endpoint}  body: {body}");
...
/// Handles nested elements with the same tag name (e.g. `<item>` inside `<item>`).
